...
6. For Cloud configuration context, use `namespace.get`, `namespace.capacity_get`, and the other advertised Namespace inspection operations when they appear in the operation enum. They are available in API-key mode and intentionally absent in mTLS mode. A permission error is evidence about the API key's Cloud control-plane scope; report it without trying another identity.
...
- Read `authentication.md` for the fixed Temporal Cloud API-key or mTLS boundary.
