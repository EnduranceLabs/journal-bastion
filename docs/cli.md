...
Journal Bastion runs inside your network and connects your MCP servers and skill
...
`npx` when the bastion starts. It is not bundled with, or installed by,
`journal-bastion`.
...
Do not put credentials directly in `bastion.json`.
...
[examples/integrations/database](https://github.com/EnduranceLabs/journal-bastion/tree/main/examples/integrations/database).

## Skills

Set `skillsDir` in `bastion.json` to publish markdown skill files alongside MCP
tools:
...
- Keep bastion tokens and integration credentials in environment variables or a
  secret manager.
- Prefer read-only database users and narrow service accounts for MCP servers.
- Run the bastion close to the systems it integrates with. It only needs
...
- [Example configs](https://github.com/EnduranceLabs/journal-bastion/tree/main/examples)
- [Common MCP servers](https://github.com/EnduranceLabs/journal-bastion/blob/main/examples/integrations/common-mcp-servers.md)
- [Config JSON Schema](https://github.com/EnduranceLabs/journal-bastion/blob/main/spec/bastion-config.schema.json)
