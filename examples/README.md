...
| [`hub-server.ts`](./hub-server.ts) | Minimal service-side server using `@journal-labs/bastion`. |
| [`hub_server.py`](./hub_server.py) | The same, using `@journal-labs/bastion`. |
...
1. **Start a client server** (the service side that bastions connect to):
   ...
   ```bash
   # TypeScript
   npm install @journal-labs/bastion
   npx tsx hub-server.ts

   # or Python
   pip install @journal-labs/bastion
