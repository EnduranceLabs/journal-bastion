...
Journal Bastion. It is intentionally not an "awesome list": entries are limited
...
## Bastion-Ready Starting Points

These options have a local `stdio` path or a remote token/header path that can
fit an unattended bastion deployment.

| Integration | Maintainer | Bastion fit | Notes |
|-------------|------------|-------------|-------|
...
| [Cloudflare MCP servers](https://developers.cloudflare.com/agents/model-context-protocol/cloudflare/servers-for-cloudflare/) | Cloudflare | Managed remote servers | Cloudflare provides managed MCP servers for docs, Workers, observability, audit logs, DNS analytics, AI Bastion, GraphQL, and other Cloudflare product areas. |
...
the non-interactive token/header path required for your bastion environment
...
| [Linear MCP server](https://linear.app/changelog/2025-05-01-mcp) | Linear | Hosted remote MCP server | Official Linear remote MCP server for Linear workspace data and project workflows. Confirm auth and token flow before server-side bastion deployment. |
...
- Prefer static token/header authentication for unattended bastion deployments.
- Use vendor OAuth flows only when they are designed for service-side or
  connector use, not only desktop IDE use.
- Split high-risk integrations into separate bastion config entries and give
  each one a narrow token or role.
- For remote MCP servers, verify whether the vendor supports Streamable HTTP,
  SSE, and non-interactive authorization in the environment where the bastion
  runs.
