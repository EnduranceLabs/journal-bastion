...
env file loaded with `journal-bastion --env-file`; do not commit credentials to
bastion config files.
...
`command`, `args`, and `envVars`. In bastion configs, `envVars` maps host
...
not be directly usable through Journal Bastion until the vendor provides a
